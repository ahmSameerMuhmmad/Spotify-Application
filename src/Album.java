...
public class Album {
        ...
        if(findSong(title)==true){
            for(Song song:songs){
                if(song.getTitle().equals(title)){
                    playList.add(song);
                    System.out.println("Song has been added to your playlist using title");
                    break;
                }
            }
        }
        else{
            System.out.println("Songs is not present in the Album");
        }
        return;
    }
    ...
    public void addToPlaylistFromAlbum(int trackNo,LinkedList<Song>playlist){
        ...
        if(index>=0 && index<songs.size()){
            ...
            System.out.println("Song has been added to your playlist using indexing(track no.)");
        }
        else{
